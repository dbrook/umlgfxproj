...
public:
        ...
        void draw() const;
        void addTri( const QVector3D &a, const QVector3D &b, const QVector3D &c,
                        const QVector3D &n );
...
void Patch::addQuad( const QVector3D &a, const QVector3D &b, const QVector3D &c,
                const QVector3D &d )
{
...
static inline QVector<QVector3D> extrude( const QVector<QVector3D> &verts,
                qreal depth )
{
...
RectTorus::RectTorus( Geometry *g, qreal iRad, qreal oRad, qreal depth, int k )
{
        ...
        for (int i = 0; i < k; ++i) {
                ...
                inside
                                << QVector3D( iRad * qSin( angle ),
                                                iRad * qCos( angle ),
                                                depth / 2.0 );
                outside
                                << QVector3D( oRad * qSin( angle ),
                                                oRad * qCos( angle ),
                                                depth / 2.0 );
        }
        ...
        for (int i = 0; i < k; ++i)
                front->addQuad( outside[i], inside[i], inside[(i + 1) % k],
                                outside[(i + 1) % k] );
        Patch *back = new Patch( g );
        for (int i = 0; i < k; ++i)
                back->addQuad( in_back[i], out_back[i], out_back[(i + 1) % k],
                                in_back[(i + 1) % k] );
        ...
        for (int i = 0; i < k; ++i)
                os->addQuad( out_back[(i + 1) % k], out_back[i], outside[i],
                                outside[(i + 1) % k] );
        parts << front << back << is << os;
}

QtLogo::QtLogo( QObject *parent, int divisions, qreal scale ) :
                QObject( parent ), geom( new Geometry() )
{
